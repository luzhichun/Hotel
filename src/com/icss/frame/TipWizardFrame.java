...
import com.icss.dao.OrderFormDao;
import com.icss.dao.OrderItemDao;
...
public class TipWizardFrame extends JFrame {
	...
	private MTable rightTable;             //右表

	private Vector<String> rightTableColumnV;      

	private Vector<Vector<Object>> rightTableValueV = new Vector<>();    //右侧表格数据

	private DefaultTableModel rightTableModel;

	private MTable leftTable;             //左表
 
	private Vector<String> leftTableColumnV;       

	private Vector<Vector<Object>> leftTableValueV = new Vector<>();     //左侧表格数据

	private DefaultTableModel leftTableModel;

	
	private Vector<Vector<Vector<Object>>> menuOfDeskV = new Vector<>();
	...
	public TipWizardFrame(final Vector user) {
		...
		Vector<Object> objects = new Vector<>();                //数组来存放每条菜单，用于显示在签单列表中
		Vector<Vector<Object>> vectors = new Vector<>();        //用来存放每一个台号的菜单
		...
		try {
			rightTableValueV = orderFormDao.queryAllForm();
		} catch (Exception e2) {
			e2.printStackTrace();
		}
		...
		rightTable.addMouseListener(new MouseAdapter() {    //右侧表格添加鼠标监听，选中某行后，在左侧自动显示该台所点菜口信息
			public void mouseClicked(MouseEvent e) {
				rightTable.getSelectedRow();    // 获得“开台列表”中的选中行
				// 清空“签单列表”中的所有行
		...
		rightTable.setRowSelectionInterval(0);
		...
		Vector<Vector<Object>> allDesks = new Vector<>();              //返回Vector<Vector<Object>>类型
		try {
			allDesks = deskDao.queryAllDesk();
			Iterator<Vector<Object>> it = allDesks.iterator();         //返回<Vector<Object>>类型
			while(it.hasNext()){
				Vector<Object> oneRow = it.next();                     //返回Object类型
				numComboBox.addItem(oneRow.get(1));
			}
		...
		numComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int openedDesk = rightTable.getRowCount();                       // 获得开台列表中的行数，即已开台数
				String item = numComboBox.getSelectedItem().toString();          // 获得“台号”下拉菜单中选中的台号
				if(openedDesk>0){                                                // 若已经有开台
					openDesk(item);
					// 清空“签单列表”中的所有行
					// 刷新“签单列表”
		...
		codeTextField.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {             // 通过键盘监听器实现智能获取菜品，每次点击一下键盘，对数据进行一次查询
				//判断如果是shift或者enter键，则不查询
				if(e.getKeyCode()!=KeyEvent.VK_ENTER && e.getKeyCode()!=KeyEvent.VK_SHIFT){
					// 输入内容不为空
					Vector<Vector<Object>> menus= new Vector<>();
					if(codeRadioButton.isSelected()){   // 按助记码查询
						String codeTxt = codeTextField.getText().trim();          // 获得输入内容
						if(codeTxt.length()>0){
							try {                                               
								menus = menuDao.queryByCode(codeTxt);            // 查询符合条件的菜品
							} catch (Exception e2) {
								e2.printStackTrace();
							}
						}else{
						...
						String numTxt= codeTextField.getText();           // 查询符合条件的菜品
						if(numTxt.length()>0){
							int num = Integer.parseInt(numTxt);
							try {
								menus = menuDao.queryByNum(num);
							} catch (Exception e1) {
								e1.printStackTrace();
							}	
						}else{
					...
					if(menus.size()>0){                                  // 存在符合条件的菜品
					   Iterator<Vector<Object>> it = menus.iterator();   // 获得第一个符合条件的菜品
					   while (it.hasNext()) {
						   Vector<Object> oneRow = it.next();
						   String sName = oneRow.get(0).toString();
						   nameTextField.setText(sName);           // 设置“商品名称”文本框为符合条件的菜品名称
						   String sUnit = oneRow.get(2).toString();
						   unitTextField.setText(sUnit);           // 设置“单位”文本框为符合条件的菜品单位	
					   }  
					}else{                                               // 不存在符合条件的菜品
					   System.out.println("不存在该菜系！");
					   nameTextField.setText("");            // 设置“商品名称”文本框为空
					   unitTextField.setText("");           // 设置“单位”文本框为空
					   pane.showConfirmDialog(null, "不存在该菜", "提示框",JOptionPane.DEFAULT_OPTION);
					}
				}
			}
		});
		...
		addButton.addActionListener(new ActionListener() {                //开单，添加新菜到签单列表
			public void actionPerformed(ActionEvent e) {
				int rightRow = rightTable.getRowCount();
				rightTable.setRowSelectionInterval(rightRow-1);
				openPill();
...
/*		checkOutButton.addActionListener(new ActionListener() {
		...
		cTopButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				UpdatePasswordDialog upDialog = new UpdatePasswordDialog();    //打开列新密码窗体
				upDialog.setLocation(
						(screenSize.width - upDialog.getWidth()) / 2,
						(screenSize.height - upDialog.getHeight()) / 2);
				upDialog.setVisible(true);                                     //传递当前用户信息
			}
		});
...
/*		if (user == null) {
	...
	private void openDesk(String item){
		int itemNum = Integer.parseInt(item);	 //台号
		try {
			Vector<Vector<Object>> opendDesk = orderFormDao.queryByNum(itemNum);
			if(opendDesk.size()>0){              //如果开台列表有台号，则选中,此处必须用size()判断，而不是！=null判断
				openPill();
				...
				// 刷新“签单列表”
				// 选中“签单列表”中的第一行	
			}else{                        //开台列表没有改台号，需要新开台
				System.out.println("需要新开台");
				String maxNum = orderFormDao.getMaxNum().toString();
				String nextNum = getNextNum(maxNum);
				int date_num = Integer.parseInt(nextNum);       //此处解析异常
				System.out.println("---" + date_num);
				orderFormDao.addForm(date_num, itemNum);
			}
	...
	public void openPill(){
		String item = numComboBox.getSelectedItem().toString(); //台号
		int itemNum = Integer.parseInt(item);
		try {
			Vector<Vector<Object>> opendDesk = orderFormDao.queryByNum(itemNum);
			Iterator<Vector<Object>> itf = opendDesk.iterator();       //从tb_order_form中调取数据
			String form_num =null;
			int formNum = 0;
			while (itf.hasNext()) {
				Vector<Object> onerow = itf.next();
				System.out.println(onerow);
				form_num = onerow.get(2).toString();        //根据台号选出订单编号
                formNum = Integer.parseInt(form_num) ;   //NumberFormatException,超出范围
				
				String col = onerow.get(3).toString();                        //从0开始计数
				int colSel = Integer.parseInt(col);
				rightTable.setRowSelectionInterval(colSel-1);
			}

			String buyName = nameTextField.getText().trim();           //获取菜名 
			if(buyName.length()>0){                //此处必须用length()判断，而不能用!=""来判断
				// 可以向签单列表中添加菜品
				int menu_num=0;                                                   
				double price=0;
				Vector<Vector<Object>> num_price = menuDao.queryByName(buyName);  //从tb_menu中调取数据
				Iterator<Vector<Object>>  it2 = num_price.iterator();
				while (it2.hasNext()) {
					Vector<Object>   oneRow2 = it2.next();                        //此处必须有这一步
					menu_num = Integer.parseInt(oneRow2.get(0).toString());       //菜品编号
					price =Double.parseDouble(oneRow2.get(1).toString()) ;          //菜品单价
				}
				String aString = amountTextField.getText().toString();            //获取点菜数量
				int amount_num = Integer.parseInt(aString);
				double tot =  amount_num*price;
				
				orderItemDao.addItem(formNum, menu_num, amount_num, tot);   //向tb_order_item添加数据				
			}else{
				System.out.println("未选菜");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}	
